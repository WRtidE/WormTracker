...
```bash
# 编辑 counter.py 底部的 TEST_VIDEO_FILE 路径，然后运行：
python counter.py
```

命令行模式下结果自动导出为同目录 `*_counts.csv` 文件。
...
本项目使用 [PyInstaller](https://pyinstaller.org/) + GitHub Actions 实现 **全自动跨平台打包**。每次推送代码或打 tag 都会自动生成：

- **Windows** → `WormTracker.exe` 单文件
- **macOS** → `WormTracker.app` 应用包

### 8.1 GitHub Actions 自动构建（推荐）
...
GitHub Actions 会自动在 Windows 和 macOS 上并行构建，产物发布到 Release 页面。
...
```bash
pyinstaller --clean wormtracker.spec
```
...
| 平台 | 输出位置 | 产物 |
|------|---------|------|
| **macOS** | `dist/WormTracker.app` | 应用包 (双击运行，可拖入 /Applications) |
| **Windows** | `dist/WormTracker.exe` | 单文件可执行文件 |

> ⚠️ PyInstaller **只能在目标平台上打包**：macOS 只能打 Mac 包，Windows 只能打 Windows 包。如需同时出两平台包，请使用 GitHub Actions 自动构建。

### 8.4 分发给用户

打包产物可直接复制到同平台机器上运行，无需安装 Python 或任何依赖。首次冷启动可能稍慢（自解压），之后即可正常使用。
